...
type EmailData = z.infer<typeof EmailBody>;

/** Inline attachment in Resend's shape: base64 content + `content_id` for `cid:` refs. */
...
interface SendTicketResult {
  sent: boolean;
  simulated: boolean;
  /** Resend message id — present on real sends only, null when the API omits it. */
  id?: string | null;
  to: string;
  orderRef: string;
}

emailRouter.post('/ticket', async (req: Request, res: Response) => {
...
async function qrAttachment(id: string, payload: string): Promise<EmailAttachment> {
...
async function sendTicketEmail(data: EmailData): Promise<SendTicketResult> {
...
// ---- presentation -----------------------------------------------------------
function labels(lang: 'mn' | 'en'): Labels {
  const mn = lang === 'mn';
  return {
...
function renderHtml(data: EmailData, t: Labels): string {
