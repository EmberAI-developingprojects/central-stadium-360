...
function fmtDate(iso: string, lang: 'mn' | 'en') {
  ...
  if (Number.isNaN(d.getTime())) return { month: '', day: 0, year: 0, time: '' };
  ...
  return (
    <div className="min-h-screen bg-[#071526] text-white">
      ...
      <header className="sticky top-0 z-50 flex items-center gap-2 sm:gap-4 h-14 sm:h-[60px] px-4 sm:px-6 md:px-7 bg-[rgba(7,21,38,0.92)] backdrop-blur-xl border-b border-white/[0.07]">
        ...
        >
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4 shrink-0">
        ...
        <div className="ml-auto flex items-center gap-2 sm:gap-3.5 shrink-0">
          ...
          <Link to="/" className="flex items-center shrink-0">
            <img src="/assets/images/brand/logo-white.png" alt="Төв Цэнгэлдэх Хүрээлэн" className="h-7 sm:h-8 w-auto" />
          </Link>
        </div>
      </header>
            ...
            <div className="bg-white/[0.04] border border-white/[0.08] rounded-2xl sm:rounded-[20px] p-5 sm:p-7 md:p-10">
              {dt?.month && (
                <div className="text-[11px] sm:text-xs font-bold tracking-[0.1em] uppercase text-white/40 mb-3 sm:mb-4">
                  {lang === 'en' ? `${dt.month} ${dt.day}, ${dt.year}` : `${dt.year} оны ${dt.month} ${dt.day}`}
                </div>
              )}
              ...
              <div className="flex items-center gap-2.5">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.4)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="shrink-0">
                  <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/>
                </svg>
                <span className="text-[13px] sm:text-sm text-white/55">{t('event_detail_location_value')}</span>
              </div>
            </div>
            ...
            <div className="bg-white/[0.05] border border-white/10 rounded-2xl sm:rounded-[20px] p-5 sm:p-7 md:p-10">
              {dt?.month && (
                <div className="mb-6 sm:mb-7">
                  ...
                  <div className="text-[15px] sm:text-base md:text-[17px] font-semibold text-white/45 mt-1.5">
                    {dt.year}{t('event_detail_year_suffix')} · {dt.time}
                  </div>
                </div>
              )}
              ...
              {event.base > 0 && (
                <div className="text-[12px] sm:text-[13px] text-white/40 mt-2.5 text-center">
                  {money(event.base)}{t('event_detail_starting_from')}
                </div>
              )}
              ...
              <div className="flex flex-col gap-4 sm:gap-5">
                <div>
                  <div className="text-[10px] sm:text-[11px] font-bold tracking-[0.14em] uppercase text-white/40 mb-1 sm:mb-1.5">{t('event_detail_location')}</div>
                  <div className="text-[13px] sm:text-sm text-white/75">{t('event_detail_location_value')}</div>
                </div>
                <div>
                  <div className="text-[10px] sm:text-[11px] font-bold tracking-[0.14em] uppercase text-white/40 mb-1 sm:mb-1.5">{t('event_detail_broadcast')}</div>
                  <div className="text-[13px] sm:text-sm text-white/75">{t('event_detail_broadcast_value')}</div>
                </div>
              </div>
            </div>
          </div>
