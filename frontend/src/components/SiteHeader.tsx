...
const HEADER_BASE_CLS =
  "w-full pt-[18px] px-6 pb-0 sticky top-0 z-[100] [backdrop-filter:blur(18px)_saturate(160%)] [-webkit-backdrop-filter:blur(18px)_saturate(160%)] border-b border-solid border-[rgba(31,41,55,0.08)] [transition:background_0.25s_ease,box-shadow_0.25s_ease] max-[920px]:px-4 max-[920px]:pt-3";
...
const TOPBAR_BASE_CLS =
  "flex items-stretch relative max-[920px]:hidden";

const TOPBAR_MAIN_CLS =
  "flex-1 text-ink rounded-[28px] flex items-center z-[1] min-w-0 py-3 px-7 gap-7 -mr-[34px] bg-[rgba(255,255,255,0.55)] [backdrop-filter:blur(14px)_saturate(160%)] [-webkit-backdrop-filter:blur(14px)_saturate(160%)] border border-solid border-[rgba(31,41,55,0.08)]";

const TOPBAR_ACCENT_CLS =
  "bg-brand-blue-tint rounded-[28px] flex items-center gap-2.5 z-[2] py-2.5 px-[22px] border border-solid border-[rgba(34,48,198,0.12)]";

const CONTACT_ITEM_CLS =
  "inline-flex items-center gap-2.5 text-[13px] font-medium whitespace-nowrap text-ink [&_svg]:w-[14px] [&_svg]:h-[14px] [&_svg]:flex-none [&_svg]:opacity-100 [&_svg]:text-brand-blue";
...
const MAINNAV_CLS =
  "flex items-center justify-between gap-6 py-[14px] px-3 max-[920px]:px-0 max-[920px]:py-3 max-[920px]:gap-3";

const LOGO_CLS =
  "inline-flex items-center gap-2.5 no-underline text-ink";
const LOGO_MARK_CLS =
  "w-auto h-11 rounded-none bg-transparent border-0 flex-none [&_img]:block [&_img]:h-full [&_img]:w-auto max-[920px]:h-9";
      ...
      <div className={HEADER_INNER_CLS}>
        ...
        <nav className={MAINNAV_CLS} aria-label="Үндсэн цэс">
          ...
          <ul className={NAV_LINKS_CLS}>
            ...
            <li><Link to="/events" className={NAV_LINK_A_CLS}>Үйл ажиллагаа &amp; Арга хэмжээ</Link></li>
            <li className={HAS_DROPDOWN_LI_CLS}>
          ...
          {NAV_GROUPS.map((group, idx) => {
            ...
            if (!group.children) {
              ...
              return (
                <li key={group.label}>
                  {isRoute ? (
                    <Link to={group.href} className={DRAWER_LINK_ROW_CLS} onClick={closeDrawer}>
                      <span>{group.label}</span>
                    </Link>
                  ) : (
                    <a href={group.href} className={DRAWER_LINK_ROW_CLS} onClick={closeDrawer}>
                      <span>{group.label}</span>
                    </a>
                  )}
                </li>
            ...
            return (
              <li key={group.label}>
                ...
                <div
                  ...
                  style={{ maxHeight: isOpen ? `${group.children.length * 44 + 16}px` : '0px' }}
                >
        ...
        <div className={DRAWER_FOOTER_CLS}>
          ...
          <div className={DRAWER_SOCIAL_ROW_CLS}>
            <a href="#" aria-label="Facebook" className={DRAWER_SOCIAL_CLS}>
              <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M13 22v-8h3l1-4h-4V7c0-1 .3-1.7 1.8-1.7H17V1.9c-.3 0-1.3-.1-2.4-.1-2.4 0-4 1.4-4 4.1V10H7v4h3.6v8z"/></svg>
            </a>
            <a href="#" aria-label="Instagram" className={DRAWER_SOCIAL_CLS}>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect x="3" y="3" width="18" height="18" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r="1" fill="currentColor"/></svg>
            </a>
