...
export default function Dashboard() {
  ...
  if (!stats) return <div className="admin-empty">Уншиж байна…</div>;
  ...
  return (
    <>
      ...
      <div className="admin-grid admin-grid-4" style={{ marginBottom: 18 }}>
      ...
      <div className="admin-grid admin-grid-2">
        <div className="admin-card">
          ...
          {recentOrders.length === 0 ? (
            <div className="admin-empty"><strong>Захиалга алга</strong>Эхний борлуулалт энд гарч ирнэ.</div>
          ) : (
            <table className="admin-table">
              ...
              <tbody>
                {recentOrders.map((o) => (
                  <tr key={o.code}>
                    <td><Link className="admin-link" to={`/admin/orders/${o.code}`}>{o.code}</Link></td>
                    <td>{o.title}</td>
                    <td>{o.user || '—'}</td>
        ...
        <div className="admin-card">
          ...
          {recentUsers.length === 0 ? (
            <div className="admin-empty"><strong>Шинэ хэрэглэгч алга</strong>Бүртгэлүүд энд гарч ирнэ.</div>
          ) : (
            <table className="admin-table">
              <thead>
              ...
              <tbody>
                {recentUsers.map((u) => (
                  <tr key={u.identifier}>
                    <td><Link className="admin-link" to={`/admin/users/${encodeURIComponent(u.identifier)}`}>{u.fullname || '—'}</Link></td>
                    <td>{u.identifier}</td>
