...
export default function Dashboard() {
  ...
  return (
    <>
      ...
      <div
        className={`${ADMIN_GRID_CLS} ${ADMIN_GRID_4_CLS}`}
      ...
      <div className={`${ADMIN_GRID_CLS} ${ADMIN_GRID_2_CLS}`}>
        <div className={ADMIN_CARD_CLS}>
          ...
          {recentOrders.length === 0 ? (
            <div className={ADMIN_EMPTY_CLS}>
              <strong>Захиалга алга</strong>Эхний борлуулалт энд гарч ирнэ.
            </div>
          ) : (
        ...
        <div className={ADMIN_CARD_CLS}>
          ...
          {recentUsers.length === 0 ? (
            <div className={ADMIN_EMPTY_CLS}>
              <strong>Шинэ хэрэглэгч алга</strong>Бүртгэлүүд энд гарч ирнэ.
            </div>
          ) : (
            <table className={ADMIN_TABLE_CLS}>
              <thead>
...
function Sparkline({ series }: { series: SparkSeries }) {
  ...
  return (
    ...
    >
      ...
      {points.map(([x, y], i) =>
        series[i].total > 0 ? (
          <circle key={i} cx={x} cy={y} r="2.5" fill="#2230C6" />
        ) : null,
      )}
    </svg>
  );
