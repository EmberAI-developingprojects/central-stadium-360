...
export default function Dashboard() {
  ...
  const bars = chartPeriod === "month" ? derived.monthlyBars : derived.weeklyBars;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
        <div>
          <h2 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: "#111", letterSpacing: "-0.02em", lineHeight: 1.2 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
          <select
            ...
            style={{ height: 36, padding: "0 10px", borderRadius: 8, border: "1px solid #e4e4e7", fontSize: 13, background: "#fff", color: "#111", fontFamily: "inherit" }}
          >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 14, alignItems: "stretch" }}>
        ...
        <div className={ADMIN_CARD_CLS} style={{ padding: "20px 24px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: "#111" }}>Ticket борлуулалт</div>
              <div style={{ fontSize: 12, color: "#71717a", marginTop: 2 }}>Сар / 7 хоногоор шүүж харна</div>
            </div>
            <select
              value={chartPeriod}
              onChange={(e) => setChartPeriod(e.target.value as "week" | "month")}
              style={{ height: 32, padding: "0 8px", borderRadius: 7, border: "1px solid #e4e4e7", fontSize: 12, background: "#fff", color: "#111", fontFamily: "inherit" }}
            >
        ...
        {/* Donut chart */}
        <div className={ADMIN_CARD_CLS} style={{ padding: "20px 24px", display: "flex", flexDirection: "column", alignItems: "center" }}>
          <div style={{ width: "100%", marginBottom: 12 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: "#111" }}>Ticket авсан хэрэглэгч</div>
            <div style={{ fontSize: 12, color: "#71717a", marginTop: 2 }}>Нийт хэрэглэгчийн хувь</div>
          </div>
          <DonutChart percent={derived.buyerPct} />
          <div style={{ width: "100%", marginTop: 16, borderTop: "1px solid #f4f4f5", paddingTop: 14, display: "flex", justifyContent: "space-between" }}>
            <div>
              <div style={{ fontSize: 11, color: "#71717a", marginBottom: 3 }}>Борлуулалт</div>
              <div style={{ fontSize: 18, fontWeight: 700, color: "#111" }}>
                {derived.uniqueBuyers.toLocaleString()}
              </div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 11, color: "#71717a", marginBottom: 3 }}>Нийт орлого</div>
              <div style={{ fontSize: 18, fontWeight: 700, color: "#111" }}>
      ...
      {/* ── Recent users table ── */}
      <div className={ADMIN_CARD_CLS} style={{ padding: 0, overflow: "hidden" }}>
        <div style={{ padding: "16px 20px", borderBottom: "1px solid #f4f4f5", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <Link to="/admin/users" style={{ fontSize: 12, color: "#2230C6", textDecoration: "none", fontWeight: 500 }}>
            Бүгдийг харах →
          </Link>
        </div>
        {derived.recentUsers.length === 0 ? (
          <div className={ADMIN_EMPTY_CLS} style={{ border: "none", borderRadius: 0 }}>
            Хэрэглэгч алга
          </div>
        ) : (
            ...
            <tbody>
              {derived.recentUsers.map((u) => (
                <tr key={u.id}>
                  <td>
                    <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
                      <div style={{
                        ...
                        {(u.fullname || u.identifier || "?").slice(0, 1).toUpperCase()}
                      </div>
                  ...
                  <td>
                    <span className={`${ADMIN_BADGE_CLS} ${u.role === "admin" ? "!bg-violet-50 !text-violet-700" : ""}`}>
                      {u.role || "user"}
                    </span>
                  </td>
                  <td style={{ color: "#71717a" }}>{(u.createdAt || "").slice(0, 10)}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
/* ────────────── Stat Card ────────────── */
function StatCard({
  label, value, badge, blue = false, icon,
}: {
  label: string;
  value: string;
  badge: number | null;
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span style={{ fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", color: blue ? "rgba(255,255,255,.7)" : "#71717a" }}>
          {label}
        </span>
        <span style={{
      ...
      <div style={{ fontSize: 30, fontWeight: 700, letterSpacing: "-0.02em", lineHeight: 1, color: blue ? "#fff" : "#111" }}>
        {value}
      </div>
      {badge !== null && (
...
/* ────────────── Bar Chart ────────────── */
type BarDatum = { label: string; revenue: number; count: number; isCurrent: boolean };
    ...
    <div style={{ position: "relative" }}>
      ...
      >
        ...
        {[0, 0.25, 0.5, 0.75, 1].map((f) => {
          ...
          return (
            <g key={f}>
              <line x1={PL - 4} y1={y} x2={totalW - PR} y2={y} stroke="#f0f0f0" strokeWidth="1" />
              <text x={PL - 6} y={y + 4} fontSize="8" fill="#aaa" textAnchor="end">
        ...
        {bars.map((b, i) => {
          ...
          return (
            ...
            >
              {/* Background bar */}
              <rect x={x} y={4} width={BAR_W} height={MAX_H} rx={5} fill="#f4f4f5" />
              {/* Value bar */}
              {barH > 0 && (
                <rect
                  x={x} y={y} width={BAR_W} height={barH} rx={5}
                  fill={b.isCurrent || isHovered
                    ? "url(#barGrad)"
                    : "#e0e3fa"}
              ...
              {/* Label */}
              <text x={x + BAR_W / 2} y={H + 18} fontSize="9" fill="#aaa" textAnchor="middle">
                {b.label}
              </text>
            </g>
          );
      ...
      {tooltip !== null && (() => {
        ...
        return (
          <div style={{
            ...
            <div style={{ color: "#aaa" }}>Борлуулалт: <b style={{ color: "#fff" }}>{b.count}</b></div>
            <div style={{ color: "#aaa" }}>Орлого: <b style={{ color: "#fff" }}>{money(b.revenue)}</b></div>
          </div>
        );
      })()}
    </div>
...
function DonutChart({ percent }: { percent: number }) {
  ...
  return (
    <svg viewBox="0 0 180 180" style={{ width: 170, height: 170 }}>
      ...
      {/* Center text */}
      <text x={cx} y={cy - 6} textAnchor="middle" fontSize="26" fontWeight="700" fill="#111">
        {percent}%
      </text>
...
function IconTicket() {
  return <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M2 9a3 3 0 1 1 0 6v2a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-2a3 3 0 1 1 0-6V7a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2v2z"/></svg>;
}
function IconRevenue() {
  return <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 1 0 0 7h5a3.5 3.5 0 1 1 0 7H6"/></svg>;
}
