#!/bin/zsh
# Launch the one-shot EC2 merge worker (Seoul, same region as 360record).
# Usage: ./launch-merge.sh <EVENT_ID>
set -euo pipefail
cd "$(dirname "$0")"
EVENT_ID="${1:?usage: launch-merge.sh <event uuid>}"
ENVFILE=../../backend/.env
...
aws ec2 run-instances --region ap-northeast-2 \
  ...
  --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=vod-merge-worker},{Key=purpose,Value=one-shot-vod-merge}]' \
  --query 'Instances[0].InstanceId' --output text

rm -f "$USERDATA"
echo "Watch progress: aws s3 cp s3://360record/wowza/_merge/log.txt - | tail -20"
