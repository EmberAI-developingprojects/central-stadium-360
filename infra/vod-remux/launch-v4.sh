#!/bin/zsh
# Launch the v4 scan-fix-merge worker. Usage: ./launch-v4.sh <EVENT_ID>
set -euo pipefail
cd "$(dirname "$0")"
...
aws ec2 run-instances --region ap-northeast-2 \
  ...
  --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=vod-v4-worker},{Key=purpose,Value=scan-fix-merge}]' \
