...
LOG=/var/log/v4.log
exec >>"$LOG" 2>&1
echo "=== v6 tolerant-fix+merge start $(date -u) ==="
...
# ---- Phase 1 (v6): tolerant re-remux of the ONE corrupt source ----
# cam3/0111f3e5 is corrupt inside the Wowza origin itself; drop the broken
# packets (brief visual skip at that point) instead of failing the merge.
VID=0111f3e5-7b04-4a23-92de-9fbcf8aff975
CAM=cam3
DUR=26855
KEY="wowza/$EVENT_ID/$CAM/$VID.mp4"
url=$(aws s3 presign "s3://$BUCKET/$KEY" --expires-in 43200)
echo "[fix/$VID] tolerant remux start $(date -u)"
if ! timeout 14400 ffmpeg -nostdin -loglevel error "${HTTP_OPTS[@]}" \
    -err_detect ignore_err -fflags +discardcorrupt+genpts -i "$url" \
    -map 0:v:0 -map 0:a:0 -c copy \
    -f mp4 -movflags frag_keyframe+empty_moov+default_base_moof - \
  | aws s3 cp - "s3://$BUCKET/wowza/$EVENT_ID/$CAM/$VID.fixed.mp4" --expected-size $(( DUR * 1830000 )); then
  echo "[fix/$VID] TOLERANT REMUX FAILED"; FAILED=1
else
  sz=$(aws s3api head-object --bucket "$BUCKET" --key "wowza/$EVENT_ID/$CAM/$VID.fixed.mp4" --query ContentLength --output text 2>/dev/null || echo 0)
  if [ "$sz" -lt $(( DUR * 1400000 )) ]; then
    echo "[fix/$VID] FIXED FILE TOO SMALL ($sz)"; FAILED=1
  else
    furl=$(aws s3 presign "s3://$BUCKET/wowza/$EVENT_ID/$CAM/$VID.fixed.mp4" --expires-in 43200)
    if scan "$furl"; then
      echo "[fix/$VID] fixed file scan CLEAN ($((sz/1000000000)) GB) — replacing original"
      aws s3 cp "s3://$BUCKET/wowza/$EVENT_ID/$CAM/$VID.fixed.mp4" "s3://$BUCKET/$KEY"
      aws s3 rm "s3://$BUCKET/wowza/$EVENT_ID/$CAM/$VID.fixed.mp4"
    else
      echo "[fix/$VID] STILL FAILS SCAN AFTER TOLERANT REMUX"; FAILED=1
    fi
  fi
fi

if [ "$FAILED" -ne 0 ]; then
  echo "=== v6 aborting before merge ==="
